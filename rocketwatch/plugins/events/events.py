...
PartialFilter = Callable[
    ...
    Coroutine[None, None, list[LogReceipt | EventData]],
]
...
class Events(EventPlugin):
        ...
        for group in config["direct"]:
            ...
            for event in group["events"]:
                ...
                try:
                    ...
                    log.exception(e)
                    log.warning(
                        f"Couldn't find event {event_name} ({event['name']}) in the contract"
                    )
                    continue
    ...
    async def trigger_event(
        ...
        try:
            default_args = {"tnx_fee": 0, "tnx_fee_usd": 0}
            event_obj = aDict(
                {
                    "event": event,
    ...
    async def get_past_events(
        ...
        log.info(
            f"Detected contract upgrade at block {contract_upgrade_block}, reinitializing"
        )
        ...
        try:
            ...
            return messages + await self.get_past_events(
                contract_upgrade_block + 1, to_block
            )
    ...
    async def process_events(
        ...
        for event in events:
            ...
            if (n := rp.get_name_by_address(event.address)) and "topics" in event:
                ...
                if event_name in [
                    "odao_upgrade_pending_event",
                    "sdao_upgrade_vetoed_event",
                    "odao_contract_added_event",
                    "odao_contract_upgraded_event",
                ]:
            ...
            identical_events = filter(
                lambda e: (
                    ...
                    and (e.blockHash == event.blockHash)
                ),
                events,
            )
            ...
            for event in tx_events:
                ...
                if full_event_name == "unstETH.WithdrawalRequested":
                    ...
                    if prev_event := tx_aggregates.get(full_event_name):
                        ...
                        _event = aDict(contract.events[event_name]().process_log(event))
                        _prev_event = aDict(
                            contract.events[event_name]().process_log(event)
                        )
    ...
    async def handle_global_event(self, event_name: str, event: aDict) -> Embed | None:
        ...
        is_minipool_event = await rp.is_minipool(event.address) or await rp.is_minipool(
            receipt.to
        )
        is_megapool_event = await rp.is_megapool(event.address) or await rp.is_megapool(
            receipt.to
        )
        ...
        if not any(
            ...
            log.warning(
                f"Skipping {event.transactionHash.hex()} because the called contract is not a minipool"
            )
            return None
        ...
        if not pubkey:
            pubkey = (
                await rp.call("rocketMinipoolManager.getMinipoolPubkey", event.address)
            ).hex()
        ...
        if (n := rp.get_name_by_address(receipt["to"])) is None or not n.startswith(
            "rocket"
        ):
    ...
    async def handle_event(self, event_name: str, event: aDict) -> Embed | None:
        ...
        if "negative_rETH_ratio_update_event" in event_name:
            args.currRETHRate = (
                solidity.to_float(args.totalEth) / solidity.to_float(args.rethSupply)
                if args.rethSupply > 0
                else 1
            )
            args.prevRETHRate = solidity.to_float(
                await rp.call(
                    "rocketTokenRETH.getExchangeRate", block=event.blockNumber - 1
                )
            )
            ...
            period_start = await rp.call(
                "rocketRewardsPool.getClaimIntervalTimeStart", block=event.blockNumber
            )
            period_length = await rp.call(
                "rocketRewardsPool.getClaimIntervalTime", block=event.blockNumber
            )
            args.rewardPeriodEnd = period_start + period_length
            # in seconds
            update_rate = await rp.call(
                "rocketDAOProtocolSettingsNetwork.getSubmitPricesFrequency",
            ...
            args.nodeAddress = await el_explorer_url(
                args.nodeAddress, block=(event.blockNumber - 1)
            )
        elif any(
            [
            ...
            assets = await rp.call(
                "RockSolidVault.convertToAssets", args.shares, block=event.blockNumber
            )
            ...
            args.address = await w3.eth.get_transaction_receipt(event.transactionHash)[
                "from"
            ]
            ...
            args.operatorList = "\n".join(
                [await el_explorer_url(address) for address in args.operators]
            )
        elif event_name in [
        ...
        if "submission" in args:
            args.submission = aDict(
                dict(zip(SUBMISSION_KEYS, args.submission, strict=False))
            )
        ...
        if "otc_swap" in event_name:
            ...
            if args.otherToken.lower() == "wETH":
                ...
                args.marketExchangeRate = await rp.call(
                    "rocketNetworkPrices.getRPLPrice"
                )
                # calculate the discount received compared to the market price
                args.discountAmount = (
                    1 - args.exchangeRate / solidity.to_float(args.marketExchangeRate)
                ) * 100
        ...
        if cfg.rocketpool.chain == "mainnet":
            ...
            args.tnx_fee_usd = round(
                await rp.get_eth_usdc_price() * args.tnx_fee / 10**18, 2
            )
        ...
        if "pdao_proposal" in event_name:
            proposal_id = (
                ...
                if "proposalID" in event.args
                else event.args.proposalId
            )
            ...
            if "add" in event_name or "destroy" in event_name:
                args.proposalBond = solidity.to_int(
                    await rp.call(
                        "rocketDAOProtocolVerifier.getProposalBond", proposal_id
                    )
                )
                ...
                args.challengeBond = solidity.to_int(
                    await rp.call(
                        "rocketDAOProtocolVerifier.getChallengeBond", proposal_id
                    )
                )
                args.challengePeriod = await rp.call(
            ...
            args.proposal_body = dao.build_proposal_body(
                ...
                include_votes=all(
                    kw not in event_name
                    for kw in ("add", "challenge", "root", "destroy")
                ),
            )
        ...
        if any(
            [
                event_name == "reth_transfer_event" and args.amount < 1000,
                event_name == "rpl_stake_event" and args.amount < 1000,
                event_name == "rpl_stake_event" and args.amount < 1000,
                event_name == "node_merkle_rewards_claimed"
                and args.ethAmount < 5
                and args.amountETH < 5,
            ...
            log.debug(
                f"Skipping {event_name} because the event ({amounts}) is too small to be interesting"
            )
            return None
        ...
        if "minipool_deposit_received_event" in event_name:
            contract = await rp.assemble_contract(
                "rocketMinipoolDelegate", args.minipool
            )
            args.commission = solidity.to_float(
                await contract.functions.getNodeFee().call()
            )
            # get the transaction receipt
            args.depositAmount = await rp.call(
                "rocketMinipool.getNodeDepositBalance",
                address=args.minipool,
            ...
            if user_deposit < args.depositAmount:
                ...
                e = (
                    await rp.get_contract_by_name("rocketVault")
                ).events.EtherWithdrawn()
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore")
                ...
                for withdraw_event in processed_logs:
                    ...
                    if (
                        event.logIndex - 7 < withdraw_event.logIndex < event.logIndex
        ...
        if "minipool_bond_reduction_started_event" in event_name:
            ...
            if event["assignment_count"] == 1:
                ...
                return None
        elif "minipool_scrub" in event_name and await rp.call(
            "rocketMinipoolDelegate.getVacant", address=args.minipool
        ):
            ...
            if event_name == "vacant_minipool_scrub_event":
                ...
                pubkey = (
                    await rp.call(
                        "rocketMinipoolManager.getMinipoolPubkey", args.minipool
                    )
                ).hex()
                ...
                if vali_info:
                    ...
                    if all(
                        [
                            vali_info["validator"]["withdrawal_credentials"][:4]
                            == "0x01",
